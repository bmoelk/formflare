...
/**
 * FreeFormer Interactive Setup Wizard & Manifest Summary
 */
...
async function main() {
  console.log(`
┌─────────────────────────────────────────────────────────────┐
│ 🔥 FreeFormer Interactive Setup Wizard                       │
└─────────────────────────────────────────────────────────────┘
`);
...
async function setupLocalDev() {
  ...
  const devVarsContent = `# FreeFormer Local Development Environment (.dev.vars)
# Generated by FreeFormer Interactive Setup Wizard
...
async function setupProdDeployment() {
  ...
  if (storageEngine === '2' || storageEngine.toLowerCase() === 'd1') {
    ...
    console.log('1. If not yet created, run: npx wrangler d1 create freeformer-db');
    console.log('2. Apply schema: npx wrangler d1 execute freeformer-db --remote --file=./schema.sql\n');
...
async function setupMultiSiteSecrets() {
  ...
  console.log('FreeFormer supports site-specific Turnstile keys, Email routes, and Webhook URLs based on siteId.\n');
