...
* `formId` *(string, required)*: Identifier for the form (e.g., `brainendeavor_contact`, `splitphase_contact`).
...
#### Request
```http
GET /submissions/brainendeavor_contact?limit=50&offset=0 HTTP/1.1
Host: your-worker.workers.dev
Authorization: Bearer YOUR_API_KEY
```
